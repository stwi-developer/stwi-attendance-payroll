...
function RunDetail(){
  ...
  return <div>
    <div className="page-head">
      ...
      <div className="actions">
        ...
        {run.status==='REVIEW'&&<button onClick={()=>api.exportRun(id!)}>Export Excel</button>}
  ...
  {visibleEmployeeSummaries.map((e: any) => (
    ...
    <section className="panel"><h3>Files &amp; Actions</h3><p>Open reviews: {reviews?.pagination.total??0}</p><div className="actions"><button onClick={()=>api.exportRun(id!)}>Export Excel</button>{run.status==='FINALIZED'?<button onClick={()=>action('reopen',()=>api.reopenRun(id!))}>Reopen</button>:<button onClick={()=>action('finalize',()=>api.finalizeRun(id!))}>Finalize</button>}</div></section>
  </div>
}
